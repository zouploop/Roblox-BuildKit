...
description: Use when building or improving ANY structure/model/environment in Roblox Studio — apartments, houses, interiors, shops, towers, terrain/landscapes, natural environments, vehicles, standalone props, whole streets, scenes & maps — especially via the roblox-buildkit MCP (rbx_capture / rbx_floor_plan / rbx_build / rbx_inspect / rbx_set_lighting). Carries the "see-as-you-build" capture loop, parametric build-script discipline, proven helpers (walls-with-openings, framed windows, balconies, parapets, stairs+floor-holes, slabs, roofs), engine-correctness rules (anchoring, z-fighting, collision, part budget), structural sealing, navigation + playtest validation, visual-fidelity & lived-in set-dressing standards, non-building guidance (terrain, organic scatter, curves, atmosphere, whole-scene composition, standalone props), proportion/material standards, and the MANDATORY QA workflow: analyze every floor, analyze every side, walk the space, enforce spacing, and ASK before using the creator store. Trigger on "build a/an X in Roblox", "make the building/apartment better", "model this", or any Studio geometry work.
---
...
Custom MCP + Studio plugin. Server lives in the repo checkout; plugin installs to
`%LOCALAPPDATA%\Roblox\Plugins\BuildKitPlugin.rbxmx`. After editing `src/*.ts` or the plugin,
run `npm run build` (compiles `dist/`, regenerates + installs the `.rbxmx`) and restart Studio.
...
| `rbx_capture(target,view,cutaway/cutawayY,zoom,isolate,annotate,contrast)` | Screenshot with scene setup applied **and guaranteed torn down** around it — that atomicity is the reason to use it over doing the same by hand in four calls. views: front/back/left/right/iso/top. `isolate`=hide all but target; `annotate`=bbox + W×H×D label; `contrast`=recolor to expose seams. OS-grab (needs Studio foreground). |
| `rbx_orbit(target,n,elevation,top,bottom,isolate,contrast)` | **Turntable:** N evenly-spaced **labeled** views orbiting the target (+top/bottom) in one contact sheet → fuse into pseudo-3D. Batches angles: `n=12` is one call vs 24. `n=4,top:true` = the old six-view montage. Pair with `rbx_describe` for exact structure. OS-grab (needs Studio foreground). |
...
| `rbx_floor_plan(target,ceilingY)` | Top-down with everything above `ceilingY` hidden = interior layout. |
...
| `rbx_isolate(mode,target)` / `rbx_annotate(mode,target)` | Bracket the official `screen_capture`: `on`=hide-the-rest / draw bbox+dims, `off`=restore. (Or just pass `isolate/annotate` to `rbx_capture`.) |
| `rbx_inspect(target)` | Bbox center/size, part count, children. |
| `rbx_describe(target,depth)` | **Text "eyes":** compact JSON tree (name/class/bbox/material/color/anchored). VERIFY builds without spending a screenshot — only capture when geometry is ambiguous. |
| `rbx_measure(a,b)` | Distance + axis delta between two points (each = instance name or `[x,y,z]`). Kills coord guessing. |
| `rbx_find(name?,exact?,class?,tag?,attr?,near?,root?,limit?)` | **Locate things without knowing the exact name.** Query by name substring, `className` (`:IsA`), CollectionService tag, attribute (key ± value), and/or proximity (near a point or instance within radius) → `{path,class,pos,size}`. Prefer over blind `execute_luau` scans; the internal name lookup is first-match only. |
...
| `rbx_gui(name,theme,root)` / `rbx_gui_preview(name,on/off)` | Build a styled ScreenGui from a component tree + edit-time CoreGui preview. |
...
1. **Study the reference first — and keep going back to it.** If a reference model exists, `rbx_inspect` it (bbox, parts) and `rbx_capture` it from every angle in `day` lighting. Note: floor count, floor height, window grid, trim, roofline, materials, proportions. **If the reference stays in the workspace, revisit it freely at any point** during the build — re-capture it next to your work to compare proportions/detail whenever you're unsure.
...
2. **Build + verify each sub-assembly in ISOLATION, then fix before composing** (the "never stack a stage on an unverified one" rule, applied to parts):
   ...
   - `rbx_describe <subassembly>` (structure) + `rbx_capture(isolate=true)` or `rbx_orbit(isolate=true)` (see it alone). Fix any split/gap NOW.
...
4. **Verify the WHOLE.** `rbx_qa <prop>` (cross-assembly overlaps + clearances), capture all sides + `rbx_orbit`, then EXERCISE every interaction — open each drawer/door in edit (`PivotTo`) or play (ProximityPrompt), and confirm it moves cleanly.
...
- Trace the player's path through the whole space mentally (and via `rbx_floor_plan`): can they reach every room, balcony, floor, and exit? If not, it's not finished.
...
- **ASK before pulling in assets**, then **analyze every one.** Before adding furniture OR any non-procedural asset (creator store OR AI-generated mesh), ASK the user which source to use. After adding, `rbx_inspect` + `rbx_capture` each asset and confirm it matches the intended result — correct scale (resize to studs), style (noir/era), and quality — before keeping it. Reject/replace mismatches. Don't add assets unprompted. See **Furniture & decor sourcing** below.
...
Three options for furniture/props/decor — confirm with the user before using, then inspect+capture every result and match scale/style/quality before keeping. Place results in the **persistent `Placed` folder** so an idempotent rebuild of the procedural geometry doesn't delete them.
...
- "Interior makes no sense / can't navigate" → partition into real rooms with doorways that actually connect; verify with `rbx_floor_plan` per floor AND walk it.
