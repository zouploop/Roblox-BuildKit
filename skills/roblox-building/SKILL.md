...
**Tool preference:** use BuildKit for staged building and inspection. Prefer `rbx_stage_render` during iteration: it is background-safe and does not touch Studio. Agent processes sharing a viewer port share Stage state; use explicit session IDs for workers. Use official Studio capture for final engine-specific checks and official play tools for runtime verification.
...
| `rbx_frame(target,view,zoom)` | Calculates coordinates for official Studio `screen_capture`. Use for engine-specific/final verification; prefer `rbx_stage_render` during Stage iteration. |
...
**Gotchas:**
- **CAPTURE PRIMARY during Stage iteration: `rbx_stage_render`.** Begin with one useful angle; use `opIndex` to frame a changed prop and `width:480,height:320` for rough overviews. Use higher resolution for seams. Inspect additional sides when they answer a new question. For final materials, lighting, CSG, or engine discrepancies, use `rbx_frame` coordinates with official Studio `screen_capture`; retain live navigation/play tests.
- **`rbx_view`, `rbx_apply({view})`, `rbx_watch`, and GUI preview screenshots use the BuildKit OS window grab**, so Studio must be visible/foreground. They crop to the reported 3D viewport when possible and fall back to the full Studio client area otherwise. Use them for composed setup/teardown, batched angles, or timed sampling; for a plain Stage shot, use `rbx_stage_render`; for a plain Studio shot, use `rbx_frame` → official `screen_capture`.
